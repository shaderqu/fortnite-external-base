...
namespace skc
{
    ...
    {
    public:
        __forceinline constexpr Encrypter(T* data)
        {
            crypt(data);
        }
...
#define better_crypt(str) Encrypt_key(str, __TIME__[4], __TIME__[7])
#define Encrypt_key(str, key1, key2) []() { \
			constexpr static auto crypted = skc::Encrypter \
				<sizeof(str) / sizeof(str[0]), key1, key2, skc::clean_type<decltype(str[0])>>((skc::clean_type<decltype(str[0])>*)str); \
					return crypted; }()
#define EncryptString(chararr) std::string(_(chararr)).c_str()
#define _(str) Encrypt_key(str, __TIME__[4], __TIME__[7])
#define Encrypt_key(str, key1, key2) []() { \
			constexpr static auto crypted = skc::Encrypter \
				<sizeof(str) / sizeof(str[0]), key1, key2, skc::clean_type<decltype(str[0])>>((skc::clean_type<decltype(str[0])>*)str); \
					return crypted; }()