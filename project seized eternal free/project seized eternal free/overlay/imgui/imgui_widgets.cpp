...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    float w = GetWindowWidth() - 10;

    const float square_sz = 15;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(w, square_sz));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    float alpha = ImTricks::Animations::FastFloatLerp(std::string(label).append("alpha"), *v, 0.f, 1.f, GetIO().DeltaTime * 5);

    Custom::Drawlist::renderFrame({ total_bb.Max.x - square_sz, total_bb.Min.y }, total_bb.Max, GetColorU32(ImGuiCol_FrameBg), GetColorU32(ImGuiCol_Border), 2);
    window->DrawList->AddRectFilled({ total_bb.Max.x - square_sz, total_bb.Min.y }, total_bb.Max, ImColor(202 / 255.f, 71 / 255.f, 70 / 255.f, alpha), 2);
    //window->DrawList->AddRectFilled({ total_bb.Max.x - square_sz, total_bb.Min.y }, total_bb.Max, ImColor(71 / 255.f, 102 / 255.f, 131 / 255.f, alpha), 2);
    
    RenderCheckMark(window->DrawList, { total_bb.Max.x - square_sz + 3, total_bb.Min.y + 3 }, GetColorU32(ImGuiCol_CheckMark, alpha), 9);

    RenderText({ total_bb.Min.x, total_bb.Min.y + total_bb.GetHeight() / 2 - label_size.y / 2 }, label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = GetWindowWidth() - 10;
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 27));
    const ImRect bb(total_bb.Min + ImVec2(0, label_size.y + 5), total_bb.Max);
    ItemSize(total_bb, style.FramePadding.y);
    ...
    Custom::Drawlist::renderFrame(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), GetColorU32(ImGuiCol_Border), g.Style.FrameRounding);
    RenderArrow({ bb.Max.x - 15, bb.Min.y + 4 }, ImGuiDir_Down, 10);

    RenderText(total_bb.Min, label);

    if (preview_value != NULL)
        RenderText({ bb.Min.x + 10, bb.Min.y + bb.GetHeight() / 2 - CalcTextSize(preview_value).y / 2 }, preview_value);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 10)); // Horizontally align ourselves with the framed text
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (item_text == label) continue;
        if (Selectable(item_text, item_selected, 0, { 0, 20 }))
        {
            value_changed = true;
            *current_item = i;
        }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + 5), total_bb.Max);
    ...
    window->DrawList->AddLine({ frame_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, { frame_bb.Max.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, ImColor(40, 40, 40));
    window->DrawList->AddLine({ frame_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, { grab_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, ImColor(202, 71, 70));
    //window->DrawList->AddLine({ frame_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, { grab_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, ImColor(71, 102, 131));
    //window->DrawList->AddCircleFilled({ grab_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, 7, ImColor(71, 102, 131));
    window->DrawList->AddCircleFilled({ grab_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, 7, ImColor(202, 71, 70));
    //Custom::Drawlist::renderCircleShadow(window->DrawList, { grab_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, 7, ImColor(71, 102, 131, 100));
    Custom::Drawlist::renderCircleShadow(window->DrawList, { grab_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 }, 7, ImColor(202, 71, 70, 55));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : 24) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(10, frame_bb.GetHeight() / 2 - CalcTextSize(buf).y / 2);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (selected)
        RenderFrame(bb.Min, bb.Max, GetColorU32(ImGuiCol_Header), false);
    ...
    RenderText({ bb.Min.x + 10, bb.Min.y + size.y / 2 - label_size.y / 2 }, label);
    // Automatically close popups
